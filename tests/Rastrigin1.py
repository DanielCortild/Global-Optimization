...
title = "ULA"
algorithm = GO.ULA_New(d=d, M=1, N=1, K=14, h=0.01, title=title, U=U, dU=dU, initial=initial)
samples_filename_ULA = algorithm.generate_samples(As=[1,2,3,4], sim_annealing=False)

# Plot empirical probabilities
postprocessor = GO.PostProcessor(samples_filename_HRLA)
...
# Create comparator
comparator = GO.Comparator([samples_filename_HRLA, samples_filename_ULA])
